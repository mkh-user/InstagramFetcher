...
def _parser() -> tuple:
    ...
    def _formatDefault(a:dict) -> dict:
        ...
        if no_false.get("usefile") and not a.get("filepath"):
            parser.error("export: -usefile can't be activate if -filepath is not set")
    ...
    exp_exc_login.add_argument("-usefile", action="store_true",
                        help="Use file for get username & password")
    ...
    exp_export.add_argument("-filepath", metavar="X",
                        help="path for source username & password file")
