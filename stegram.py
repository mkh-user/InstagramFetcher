...
class _instagram:
    ...
    def __init__(self, o:object, orgtar:str ,**kwargs:dict) -> None:
        global _
        _ = o
        if not kwargs.get("usefile"):
            self.usefile = kwargs.get("usefile")
            ...
            self.max = kwargs.get("max") if  kwargs.get("max") else -1
            print(self.max)
            ...
            book = openpyxl.load_workbook(kwargs.get("filepath"))
            ...
            for i in range(1, r+1):
                ...
                self.default_h = {'User-Agent':USER_AGENT}
                sessionid = _credToSessID([sheet.cell(row=i+1, column=1).value, sheet.cell(row=i+1, column=2).value])
